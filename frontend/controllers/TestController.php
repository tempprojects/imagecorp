     ...
     public function actionResult($test) {
        ...
        foreach ($tests[$current_test]['tests'] as $testResult) {
            ...
            if ($testModel->getAttribute('result_type_id') == 1) {
                ...
                $query = new Query;
                $query->select('answer, query_values, query_colors')->from('test_values')->where(['and', "`from`<=$result", "`to`>=$result"])->andWhere(['test_id' => $testResult['test_id']]);
        ...
        // parse query products
        $model = new Test;
        $query_sort = $model->sort(\Yii::$app->request->queryParams);
        $count = 100;
        foreach ($resultRender as $res) {
            if ($res['result']['query_colors'] != '') {
                $query_colors = implode(' OR ',explode(',', str_replace(' ', '', strtolower($res['result']['query_colors']))));
                $query = '('.implode(' OR ', explode(',', $res['result']['query_values'])) .') OR ('.$query_colors. ')';
                $url = "http://api.gdeslon.ru/api/search.xml?q=" . $query . "&l=" . $count . "&p=1&_gs_at=d6c372061a5f4acafe310c1a9a2bbbfbfff0a3f6";
            } else {
                $query = implode(' OR ', explode(',', $res['result']['query_values']));
                $url = "http://api.gdeslon.ru/api/search.xml?q=" . $query . "&l=" . $count . "&p=1&_gs_at=d6c372061a5f4acafe310c1a9a2bbbfbfff0a3f6";
            }
        }
