...
<div class="test-values-form">
    ...
    <div class="form_section" data-testvalue="<?= $mod->getAttribute('id') ?>">
        <div class="row">
            ...
            <?php endif; ?>
            <div class="col-md-<?= $result_type==1?3:($result_type==1 || $result_type==4)?> col-lg-<?= ($result_type==1 || $result_type==4)?3:3?>">
                <?= $form->field($mod, '[' . $key . ']answer')->textInput([ 'required' => 'required']) ?>
            </div>
            <div class="col-md-<?= $result_type==1?3:5?> col-lg-<?= $result_type==1?3:4?>">
                <?= $form->field($mod, '[' . $key . ']query_values')->textarea(['rows' => 1, 'required' => 'required']) ?>
            </div>
            <div class="col-md-<?= $result_type==1?3:5?> col-lg-<?= $result_type==1?3:4?>">
                <?= $form->field($mod, '[' . $key . ']query_colors')->textarea(['rows' => 1]) ?>
            </div>
